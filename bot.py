...
# Значения берутся из переменных окружения (BOT_TOKEN, ADMIN_ID) — так их
...
# ---------- Фиктивный веб-сервер (нужен только для хостинга на Render Free) ----------
# Render Free даёт бесплатные часы только Web Service, а не фоновым процессам.
# Поэтому бот дополнительно поднимает мини HTTP-сервер на порту из переменной
# PORT — просто чтобы Render видел открытый порт и считал сервис веб-сервисом.
# Держать сервис "живым" (не давать засыпать через 15 мин простоя) должен
# внешний пингер (например, UptimeRobot), который стучится на этот адрес
# каждые 5-10 минут. Если запускаешь бота не на Render (например, на VPS
# через systemd) — этот сервер безвреден, просто игнорируй его.
#
# Используем встроенный в Python http.server, а не Flask — без внешней
# зависимости и без её нюансов с запуском не в главном потоке.
import threading
...
def _run_keepalive_server():
    ...
    print(f"Keepalive HTTP-сервер слушает порт {port}")
...
if __name__ == "__main__":
    try:
        ...
        print(f"⚠️ Не удалось запустить keepalive-сервер: {e!r}")
